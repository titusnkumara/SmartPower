...
static void ICACHE_FLASH_ATTR tcpclient_sent_cb(void *arg)
{
	...
	list_lwip_tcp_psc();
	#endif
	//espconn_disconnect(pespconn); //this should not disconnect the connection
}
...
static void handleData(char * pdata){
	#ifdef PLATFORM_DEBUG
	ets_uart_printf("handling data\r\n");
	ets_uart_printf("%s\r\n",pdata);
	#endif
	...
	if(strcmp(pdata,"ON")==0){

		uint8_t state=1;

		GPIO_OUTPUT_SET(LED_GPIO, state);
		ets_uart_printf("error with data state:%d\r\n",state);
		wdt_feed();
		os_sprintf(stateOfDevice,"ON");


	}
	else if(strcmp(pdata,"OFF")==0){
		uint8_t state=0;

		GPIO_OUTPUT_SET(LED_GPIO, state);
		ets_uart_printf("error with data state:%d\r\n",state);
		wdt_feed();
		os_sprintf(stateOfDevice,"OFF");

	}else{
		ets_uart_printf("error with data\r\n");
	}
...
/*
 * tcpconnection recive callback
 * recived data will be available int pdata variable
 *
 * */
static void ICACHE_FLASH_ATTR tcpclient_recv_cb(void * arg, char * pdata, unsigned short len)
{
	struct espconn *pespconn = arg;
	#ifdef PLATFORM_DEBUG
	ets_uart_printf("tcpclient_recv_cb\r\n");
	ets_uart_printf("%s\r\n",pdata);
	#endif
	#ifdef LWIP_DEBUG
	list_espconn_tcp(pespconn);
	...
	//we have data on pdata, should process it
	handleData(pdata);


	//after reciving we should diconnect the connection
...
static void ICACHE_FLASH_ATTR tcpclient_connect_cb(void *arg)
{
	...
	os_sprintf(payload,"{\"id\":\"%s\",\"state\":\"%s\"}",DEVICE_ID,stateOfDevice);
...
static void ICACHE_FLASH_ATTR senddata()
{
	console_printf("Send Data initialized :) \r\n");
	os_timer_disarm(&WiFiLinker);
	char info[150];
	char tcpserverip[15];
